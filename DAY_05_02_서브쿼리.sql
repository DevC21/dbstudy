...
/*
    ...
    3. SELECT 문의 실행 순서를 조정할 때 사용 할 수 있다
*/
...
-- 2. 내림차순 결과에 행 번호(ROWNUM)을 붙인다 -- 별명 B
...
-- 4. 연봉 TOP 10을 조회하시오.
-- 1) 연봉의 내림차순 정렬하고 행 번호(ROW_NUMBER 함수)를 붙인다. -- 별명 A
-- 2) 행번호가 1부터 10인 테이블을 조회한다.
SELECT A.*
...
-- 1. (비상관)부서번호가 50인 부서에 근무하는 사원의 사원번호, 사원명, 부서명을 조회하시오.
SELECT EMPLOYEE_ID
     , LAST_NAME
...
/*
    ...
    2. 테이블을 복사하는 용도로도 사용 할 수있다.
    3. 형식
        CREATE TABLE 테이블명 (칼럼1, 칼럼2, ...)
        AS (SELECT 칼럼1, 칼럼2, ...)
*/

-- 1. 사원번호, 사원명, 연봉, 부서번호를 조회하고 결과를 새 테이블로 생성하시오.

DROP TABLE EMP_T2;
CREATE TABLE EMP_T2(EMP_NO, NAME, SALARY, DEPART)
AS (SELECT EMP_NO, NAME, SALARY, DEPART
    FROM EMP_T);
    
-- 2. 부서 테이블의 구조만 복사하여 새 테이블을 생성하시오.
CREATE TABLE DEPT_T2(DEPT_NO, DEPT_NAME, LOCATION)
...
INSERT INTO DEPT_T2(DEPT_NO, DEPT_NAME, LOCATION) 
(SELECT DEPT_NO, DEPT_NAME, LOCATION 
FROM DEPT_T 
WHERE LOCATION = '대구');

INSERT INTO DEPT_T2 (SELECT * FROM DEPT_T WHERE LOCATION = '대구');
COMMIT;
