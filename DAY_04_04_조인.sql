/*
    ...
    3. 조인 종류
        ...
        2) 외부 조인 : 어느 한 테이블에만 존재하는 데이터를 조회 대상으로 
*/

/* 
    내부 조인(INNER JOIN)
    1. 조인하는 두 테이블에 모두 존재하는 데이터만 조회한다.
    2. 어느 한 테이블에만 존재하는 데이터는 조회되지 않는다.
    3. ANSI 문법
        SELECT 조회할 칼럼, ...
        FROM 테이블1 INNER JOIN 테이블2 -- 보통 PK를 테이블1로 두고 FK를 테이블2로 둔다.
        ON 조인 조건 -- 조인 조건도 위와 동일하게 작성한다.
*/

-- 1. 사원번호, 사원명, 부서번호, 부서명을 조회하시오.
SELECT E.EMP_NO AS 사원번호, E.NAME AS 사원명, E.DEPART AS 부서번호, D.DEPT_NAME AS 부서명
FROM DEPT_T D INNER JOIN EMP_T E
ON D.DEPT_NO = E.DEPART;

-- 2. 부서별 평균 연봉을 조회하시오. 부서명과 평균 연봉을 조회하시오.
SELECT D.DEPT_NAME AS 부서명, AVG(E.SALARY) AS 평균연봉
FROM DEPT_T D INNER JOIN EMP_T E
ON D.DEPT_NO = E.DEPART
...
-- 외부 조인 확인을 위한 데이터 입력
INSERT INTO EMP_T VALUES(EMP_SEQ.NEXTVAL, '홍길동', NULL, '회장', 'F', '00/01/01', 10000000);
COMMIT;
...
-- 2. 부서별 사원수를 조회하시오. 사원이 없으면 0으로 조회하시오
...
SELECT D.DEPARTMENT_NAME, FLOOR(NVL(AVG(E.SALARY), 0))    
...
-- 1                            N
-- 매니저 테이블 : EMPLOYEES M   사원테이블 : EMPLOYEES E
-- PK           : EMPLOYEES_ID  FK         : MANAGER_ID

SELECT E.EMPLOYEE_ID AS 사원번호
     , E.FIRST_NAME || ' ' || E.LAST_NAME AS 사원명
     , E.MANAGER_ID AS 매니저번호
     , M.FIRST_NAME || ' ' || M.LAST_NAME AS 매니저명
FROM EMPLOYEES M INNER JOIN EMPLOYEES E
ON M.EMPLOYEE_ID = E.MANAGER_ID;

-- 9. 같은 부서내에서 나보다 급여를 더 많이 받는 사원(들)을 조회하시오.
-- 관계
-- 나는 여러 사원과 관계를 맺는다.
-- 나(EMPLOYEES ME)              다른 사원들(EMPLOYEES YOU)
-- 같은 부서의 사원만 조인하기 위해 부서 번호로 조인조건을 생성함

SELECT ME.EMPLOYEE_ID AS 사원번호
     , ME.LAST_NAME AS 사원명
     , ME.SALARY AS 급여
     , YOU.EMPLOYEE_ID AS 다른사원번호
     , YOU.LAST_NAME AS 다른사원명
     , YOU.SALARY AS 다른사원급여
