...
CREATE TABLE CUSTOMER_T(
    ...
    CUST_AGE NUMBER(3) CHECK(CUST_AGE >= 0 AND CUST_AGE <= 120),
                     --CHECK(CUST_AGE BETWEEN 0 AND 120), 같은 방법
...
ALTER TABLE CUSTOMER_T ADD CUST_GRADE VARCHAR2(20 BYTE) CHECK(CUST_GRADE = 'VIP' OR CUST_GRADE = 'GOLD' OR CUST_GRADE = 'SILVER' OR CUST_GRADE = 'BRONZE');
                                                      --CHECK(CUST_GRADE IN('VIP', 'GOLD', 'SILVER', 'BRONZE'));
...
/*
    ...
    2. FK
        1) 추가     :  ALTER TABLE 자식테이블명 ADD CONSTRAINT 제약조건 FOREIGN KEY(칼럼명) REFERENCES 부모테이블(참조칼럼명) [옵션]
...
/*
   FK_CUSTOMER_BANK 제약조건이 비활성화되었기 때문에 
   BANK_T에 없는 은행 코드인 '12345'이 정상 삽입됨
*/
INSERT INTO CUST_T VALUES(1, 'TOM', '111-111', '12345', 'VIP');

-- FK_CUSTOMER_BANK 제약조건을 활성화하시오.
-- 위에서 삽입한 데이터가 참조 무결성을 위배하기 때문에 활성화되지 않음
