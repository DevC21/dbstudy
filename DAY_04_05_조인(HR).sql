...
SELECT D.DEPARTMENT_NAME AS 부서명, COUNT(E.DEPARTMENT_ID) AS 사원수, FLOOR(AVG(E.SALARY)) AS 평균연봉
FROM DEPARTMENTS D INNER JOIN EMPLOYEES E
ON D.DEPARTMENT_ID = E.DEPARTMENT_ID
GROUP BY D.DEPARTMENT_NAME;

-- 5. JOB_HISTORY 테이블에 존재하는 사원들을 대상으로 사원번호, 현재 JOB_ID, 예전 JOB_ID를 조회하시오.
SELECT E.EMPLOYEE_ID AS 사원번호, E.JOB_ID AS 현재JOB_ID, J.JOB_ID AS 예전JOB_ID
