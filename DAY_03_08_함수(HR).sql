...
SELECT EMPLOYEE_ID AS 사번, FLOOR(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) AS 근무개월수
FROM EMPLOYEES;
...
SELECT JOB_ID
     ...
     , SUBSTR(JOB_ID, 4, LENGTH(JOB_ID)) AS 직급
FROM EMPLOYEES;
...
-- 100  STEVEN KING
-- 101  NEENA KOCHHAR
SELECT EMPLOYEE_ID AS 사번, UPPER(FIRST_NAME || ' ' || LAST_NAME) AS 이름
FROM EMPLOYEES;
