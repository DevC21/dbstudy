/*
    ...
    3. DQL를 실행해도 데이터베이스에 변화는 발생하지 않는다.
    4. 형식
...
SELECT * -- 모든 칼럼을 의미하는 *, 실제 개발할때는 사용 금지(성능 저하 발생)
FROM DEPT_T;

SELECT DEPT_T.DEPT_NO, DEPT_T.DEPT_NAME, DEPT_T.LOCATION
FROM DEPT_T;

SELECT D.DEPT_NO, D.DEPT_NAME, D.LOCATION
FROM DEPT_T D; -- 테이블의 별명(ALIAS)을 D로 지정

-- 2. 부서 테이블의 모든 칼럼을 조회하시오. 조회할 칼럼명을 '부서번호', '부서명', '위치'로 지정하시오.
SELECT D.DEPT_NO AS 부서번호, -- 칼럼의 별명(ALIAS)을 부서번호로 지정
       D.DEPT_NAME AS 부서명, 
       D.LOCATION AS 위치
FROM   DEPT_T D;
...
-- 7. 사원 테이블에서 기본급이 200만에서 300만원인 사원을 조회하시오.
SELECT *
FROM EMP_T
