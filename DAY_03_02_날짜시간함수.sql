/*
    1. DATE 형식의 날짜/시간 연산
        ...
        2) 단위 처리
            ...
            (4) 1/24/60/60 : 1초
*/
SELECT TO_CHAR(SYSDATE, 'YY/MM/DD HH24:MI:SS') -- 현재 날짜
...
SELECT EXTRACT(YEAR FROM SYSDATE) AS 년
     ...
     , EXTRACT(HOUR FROM SYSTIMESTAMP) + 9 AS "UTC+9"
...
-- 1월 1일 부터 몇개월 지났는지 계산하기
SELECT MONTHS_BETWEEN(SYSDATE, TO_DATE('2023/01/01'))
FROM DUAL;